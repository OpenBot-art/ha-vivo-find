...
| `sensor.iqooo_address` | 传感器 | 文字地址，例如「武重四街坊，湖北省武汉市武昌区…」。属性里带 `locate_status` / `address_age_s`，为空时会给出原因 |
...
> 实体 ID 会按设备别名生成。你的设备别名实际是 `iQOOO`，所以是
> `device_tracker.iqooo`。在「开发者工具 → 状态」里搜 `vivo_find` 或设备名可确认。
...
`person` 判定和默认地图底图全部基于 WGS84。所以**不转换的话，地图上的点会偏约 1220 米**
（武汉实测：lng +0.011919 / lat +0.003872）。

本集成默认已按 `BD-09 → WGS84` 转换，装在 HA 上就是对的。

### 这个结论是怎么来的

vivo 返回 `114.350962, 30.558978`，自报地址「武重四街坊，…武昌区水果湖街道东湖路104号」。
把原值按三种假设分别换算成"高德应该显示的点"，再跟高德的门牌级数据比：

| 参照点（高德 GCJ-02） | 按 BD-09 解释 | 按 GCJ-02 解释 | 按 WGS84 解释 |
|---|---|---|---|
| 东湖路104号 **门址** | **74 m** ✅ | 994 m | 1296 m |
| 武重四街坊 POI | **91 m** ✅ | 927 m | 1179 m |
| 武重宿舍 POI | **84 m** ✅ | 847 m | 1140 m |

补充两条硬证据：

1. **门牌归属**：vivo 报的是「东湖路104号」。高德数据显示「中北路四街坊」的门牌是
   **东湖路97号**、「仙茶(东湖路店)」是**东湖路91号** —— 同一门牌段，都落在 BD-09 候选点
   100~160 米内。而另一个候选点对应的「复地·东湖国际四期」门牌是**兴武路1号**，
   根本不在东湖路上。
2. **官方接口交叉**：高德官方坐标转换（`coordsys=baidu`）把 vivo 原值转成
   `114.344537, 30.552745`，与本集成本地实现只差 **0.11 米**。

> 排查踩坑记录：最初只用「复地·东湖国际四期」这一个 POI 做锚点，它距原值仅 121 米，
> 差点误判成 GCJ-02。后来发现那是巧合 —— BD-09 的偏移量恰好把「武重四街坊」搬到了
> 「复地·东湖国际」附近，而这两个 POI 本来相距就有约 1 公里。
...
开发者工具 → 状态 → device_tracker.iqooo → 属性
  raw_coordinates: 114.350962,30.558978   ← vivo 给的 BD-09
  coordinates:     114.339043,30.555106   ← 实际画在 HA 地图上的 WGS84
  source_crs:      bd09
  target_crs:      wgs84
```
...
- 坐标系结论（BD-09）是 2026-09-18 在武汉实测得出的，接口内部没有坐标系标识字段。
  如果哪天位置又开始偏，跑 `dev/check_crs.py` 重新判定，改配置即可
